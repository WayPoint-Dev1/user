...
public class UserMappingMapper {
  ...
  public static Mono<Tuple2<String, UUID>> validateUserTripMappingRequest(
      ServerRequest serverRequest) {
    String username = serverRequest.pathVariable("username");
    String tripId = serverRequest.pathVariable("tripId");
    log.info(
        "validateUserTripMappingRequest :: username :: {} :: tripId :: {}", username, tripId);
  ...
  public static UserTripMap deleteUserTripMap(UserTripMap userTripMap) {
    userTripMap.setIsActive(false);
    userTripMap.setUpdatedOn(LocalDateTime.now());
    return userTripMap;
  }
}
